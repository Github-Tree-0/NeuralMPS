...
class Classifier(nn.Module):
    def __init__(self, batchNorm, c_in, other):
        ...
        self.linear = torch.nn.Linear(other['ints_cls'], 1)
    ...
    def forward(self, inputs):
        ...
        if self.other['s1_est_i']:
            ...
            temp_ints = self.int_est(out)
            # outputs['ints'] = self.linear(temp_ints.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
            outputs['ints'] = self.linear(temp_ints.permute(0, 2, 3, 1)).permute(0, 3, 1, 2).abs()
        return outputs
...
class LCNet(nn.Module):
    ...
    def prepareInputs(self, x):
        ...
        if self.other['in_light']:
            light = x[idx].expand_as(imgs)
            light = torch.split(light, 3, 1)
    ...
    def forward(self, x):
        ...
        if self.other['s1_est_i']:
            ...
            #pred['ints'] = pred['ints'] / (torch.norm(pred['ints']) + 1e-8) # uniformalization
            pred['intens'] = self.convertMidIntens(pred, len(inputs))
            # pred['intens'] = pred['ints']
        return pred
