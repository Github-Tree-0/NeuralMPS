...
def get_corr(gt_int, pred_int):#计算两个向量person相关系数
    gt_int, pred_int = gt_int.reshape(-1), pred_int.reshape(-1)
    gt_mean, pred_mean = torch.mean(gt_int), torch.mean(pred_int)
    corr = (torch.sum((gt_int - gt_mean) * (pred_int - pred_mean))) / (
                torch.sqrt(torch.sum((gt_int - gt_mean) ** 2)) * torch.sqrt(torch.sum((pred_int - pred_mean) ** 2)))
    return {'corr': corr.abs().item()}
